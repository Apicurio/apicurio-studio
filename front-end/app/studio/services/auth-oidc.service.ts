...
export class OIDCDirectGrantAuthenticationService implements IAuthenticationService {
    ...
    public login(user: string, credential: any): Promise<User> {
        ...
        let headers: Headers = new Headers({ "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded" });
        ...
        return this.http.post(authTokenUrl, body, options).map( response => {
            console.info("[OIDCDirectGrantAuthenticationService] Received access token.");
            let token: OIDCDirectGrantAccessToken = response.json() as OIDCDirectGrantAccessToken;

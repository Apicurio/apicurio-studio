...
export class NewOperationCommand extends AbstractCommand implements ICommand {
    ...
    public execute(document: OasDocument): void {
        ...
        pathParamNames.forEach( paramName => {
            ...
            if (!this.isNullOrUndefined(siblingParam)) {
                if (!this.isNullOrUndefined(siblingParam.$ref)) {
                    param.$ref = siblingParam.$ref;
                }
                if (!this.isNullOrUndefined(siblingParam.description)) {
                    param.description = siblingParam.description;
                }
                if (!this.isNullOrUndefined(siblingParam.type)) {
                    param.type = siblingParam.type;
                }
                if (!this.isNullOrUndefined(siblingParam.schema)) {
                    param.schema = param.createSchema();
                    if (!this.isNullOrUndefined(siblingParam.schema.$ref)) {
    ...
    private detectSiblingParameter(path: Oas20PathItem, paramName: string): Oas20Parameter {
        ...
        for (let op of ops) {
            if (this.isNullOrUndefined(op)) { continue; }
